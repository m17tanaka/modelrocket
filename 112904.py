...
csv_writer.writerow(['Time', 'Temperature', 'Pressure', 'Elevation', 'GNGGA'])
...
def animate(frame):
    ...
    try:
        ...
        print("Received Line:", received_line)  # データの確認

        # データの分割
        parts = received_line.split(',')
        if len(parts) == 4:  # 4つのデータが正しく分割できたと仮定
            tmp, pressure, _, gpsSentence = map(float, parts)
            ...
            # CSVファイルにデータを追加
            csv_writer.writerow([current_time, tmp, pressure, elevation, gpsSentence])
